    ...
    <>
      <div style={{ fontSize: 16, display: "flex", flexDirection: "row-reverse", alignItems: "center", justifyContent: 'space-between', width: "100%" }}>
        <div style={{ display: "flex", flexDirection: "column", }}>
          <span style={{}}>{t("Ngày bắt đầu")} : <span style={{ color: "green" }}>{moment(props.dayflight, "DD-MM-YYYY").format("DD MMM YYYY")}</span></span>
          <span style={{}}>{t("Ngày kết thúc")} : <span style={{ color: "green" }}>{moment(props.daydestination, "DD-MM-YYYY").format("DD MMM YYYY")}</span></span>
        </div>
        <div>
          <div onClick={() => {
          ...
          }} style={{ fontSize: 17, fontWeight: 600, color: "#2e89ff", cursor: "pointer" }}>{props.data_id.toString() === props.detail.toString() ? t("Thu gọn") : t("Chi tiết")}</div>
        </div>
      </div>
      <div style={{ width: "100%", height: props.detail.toString() === props.data_id.toString() ? "auto" : 0, transition: "all 0.2s linear", background: "#fff", marginTop: 8, borderRadius: 10, overflow: "hidden", boxSizing: "border-box", padding: props.detail.toString() === props.data_id.toString() ? 10 : 0 }}>
        <div>{t("Thông tin chuyến bay")}</div>
        <br />
        <div style={{ fontSize: 18, fontWeight: 600 }}>{t("Số hành khách đã đặt vé")}: {data}&nbsp;&nbsp;&nbsp;&nbsp; {<span onClick={() => {
          setdetail_(prev => !prev)
          if (detail_ === false) {
            get_customer_book(props.id_flight, setdatacustomer)
          }
        }} style={{ color: "#2e89ff", cursor: "pointer" }}>{detail_ === true ? t("Thu gọn") : t("Chi tiết")}</span>} </div>
        {
          detail_ === true &&
          <>
            {
                ...
                <React.Fragment key={key}>
                  {
                    ...
                    <div>
                      <div>{t("Kiểu khách hàng")}: <strong>{item?.type_user}</strong></div>
                      <div>{t("Họ tên đầy đủ")}: <strong>{item?.name} {item?.surname}</strong></div>
                      <div>Email: <strong>{item?.email}</strong></div>
                      <div>{t("Đã đặt chuyến bay lúc")}: <strong>{item?.time_book}</strong></div>
                    </div>
                  }
                  <br />
                  {
                    ...
                    <div>
                      <div>{t("Kiểu khách hàng")}: <strong>{item?.type_user}</strong></div>
                      <div>{t("Họ tên đầy đủ")}: <strong>{item?.name} {item?.surname}</strong></div>
                      <div>{t("Ngày tháng năm sinh")}: <strong>{item?.date_birth}/{item?.month_birth}/{item?.year_birth}</strong></div>
                      <div>{t("Quốc tịch")}: <strong>{item?.nationality}</strong></div>
        ...
        <br />
        <div style={{ fontSize: 18, fontWeight: 600 }}>{t("Thời gian bay")}:
