...
export const ChooseTimeAndPlace = (props) => {
  ...
  return (
    <div className={"su-1 " + props.className || ""}>
      <Mode category={t("Một chiều / Khứ hồi")} />
...
const SearchFlight = (props) => {
  ...
  return (
    <div className="su-2">
      ...
      <div className="cs-1">
        <div className="wo-1">{t("passenger")}</div>
        <div
          className="gr-1"
          ref={myRef}
          onClick={() => setopensuggest(() => true)}
          style={{
            outline: opensuggest === true ? "2px solid #2e89ff" : "none",
          }}
        >
          <div className="ic-1">
            <img
          ...
          <input
            type="text"
            value={`${props.data?.adult} người lớn, ${props.data?.kid} trẻ em, ${props.data?.baby} em bé`}
            spellCheck={true}
            aria-invalid={false}
      ...
      <ClassSeatComponent
        value={props.seatclass}
        setValue={props.setseatclass}
      />
      {props.notFindFlight !== true && (
        ...
        >
          <Link
            to={`/flight/fullsearch?ap=${props.origin?.code_airport}.${
              props.destination?.code_airport
            }&dt=${moment(new Date(props.startdate)).format("DD-MM-YYYY")}&ps=${
              props.data.adult
            }.${props.data.kid}.${props.data.baby}&sc=${
              props.seatclass.e
            }&roundtrip=${
              props.check === true
                ? moment(new Date(props.arrivaldate)).format("DD-MM-YYYY")
                : "none"
            }`}
            className="dkw-1"
          >
...
const ComponetSearch = () => {
  ...
  return (
    <div className="ia-1">
      <img
        src="https://d1785e74lyxkqq.cloudfront.net/_next/static/v2/6/6109dccccb4bbae97f5ded035b3853d9.svg"
        alt="open"
      />
      <div className="so-1" style={{ whiteSpace: "nowrap" }}>
        {t("Tìm chuyến bay")}
      </div>
    </div>
  );
